...
@section('page-content')
    <div class="card mt-5" style="width: 100%;">
        <div class="card-body">
            <p class="text-bold" style="background:#dee2e6; padding: 1.2em;">Update Pricing</p>
            @if ($errors->any())
            ...
            <form action="{{ route('dashboard.pricing.update',$pricing->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" type="number" name="min_weight" placeholder="min weight" id="example-name" value="{{$pricing->min_weight}}">
                </div>
                <div class="form-group">
                    <label for="example-email" class="form-control-label">Max Weight</label>
                    <input class="form-control" type="number" name="max_weight" placeholder="max weight" id="example-email" value="{{$pricing->max_weight}}">
                </div>
                <div class="form-group">
                    <label for="example-phone" class="form-control-label">Price</label>
                    <input class="form-control" type="number" name="price" placeholder="price" id="example-phone" value="{{$pricing->price}}">
                ...
                <div class="form-group">
                    ...
                    <select name="courier_type_id" class="form-control">
                        ...
                        @foreach($courier_types as $courier_type)
                            <option value="{{$courier_type->id}}" {{$courier_type->id==$pricing->courier_type_id?'selected':''}}>{{$courier_type->title}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="delivery_mode_id" class="form-control">
                        ...
                        @foreach($delivery_modes as $delivery_mode)
                            <option value="{{$delivery_mode->id}}" {{$delivery_mode->id==$pricing->delivery_mode_id?'selected':''}}>{{$delivery_mode->title}}</option>
                        @endforeach
                    </select>
                </div>
                @php
                ...
                <div class="form-group">
                    ...
                    <select name="user_id" class="form-control">
                        ...
                        @foreach($users as $user)
                            <option @if($user->id == $pricing->user_id) selected @endif value="{{$user->id}}">{{$user->name}}</option>
                        @endforeach
                    </select>
                </div>
                <!--<div class="form-group">
                    ...
                    <select name="sender_city_id" class="form-control">
                        ...
                        @foreach($cities as $city)
                            <option value="{{$city->id}}" {{$city->id==$pricing->sender_city_id?'selected':''}}>{{$city->name}}</option>
                        @endforeach
                    </select>
                </div>-->
                <!--<div class="form-group">
                    ...
                    <select name="receiver_city_id" class="form-control">
                        <option value="" selected>Choose...</option>
                        @foreach($cities as $city)
                            <option value="{{$city->id}}" {{$city->id==$pricing->receiver_city_id?'selected':''}}>{{$city->name}}</option>
                        @endforeach
                    </select>
                </div>-->
                <div>
