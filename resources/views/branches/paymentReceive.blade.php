                    ...
                    @foreach($branches as $branch) 
                        <tr>
                            ...
                            <td>
                                ...
                                @php
                                    $delivery_hub_cod = \App\Models\CourierRequest::where('delivery_hub',$branch->id)->where('status_id','18')->where('hub_payment','yes')->sum('cash_on_delivery_amount');

                                    $delivery_hub_merge_cod = \App\Models\CourierRequest::join('pricings','pricings.id','courier_requests.pricing_id')->where('courier_requests.status_id','18')->where('delivery_hub',$branch->id)->where('hub_payment','yes')->where('courier_requests.paid_by','merged_with_cod')->sum('pricings.price');

                                    $delivery_hub_cod_amount = $delivery_hub_cod - $delivery_hub_merge_cod;

                                    $pickup_hub_cod = \App\Models\CourierRequest::where('status_id','18')->where('delivery_hub','0')->where('branch_id',$branch->id)->where('hub_payment','yes')->sum('cash_on_delivery_amount');

                                    $pickup_hub_cod_merge_cod = \App\Models\CourierRequest::join('pricings','pricings.id','courier_requests.pricing_id')->where('courier_requests.status_id','18')->where('delivery_hub','0')->where('branch_id',$branch->id)->where('hub_payment','yes')->where('courier_requests.paid_by','merged_with_cod')->sum('pricings.price');

                                    $pickup_hub_cod_amount = $pickup_hub_cod - $pickup_hub_cod_merge_cod;

                                    $delivery_hub_charge = \App\Models\CourierRequest::join('pricings','pricings.id','courier_requests.pricing_id')->where('courier_requests.status_id','18')->where('delivery_hub',$branch->id)->where('hub_payment','yes')->where('paid_by','!=','sender')->sum('pricings.price');

                                    $pickup_hub_charge = \App\Models\CourierRequest::join('pricings','pricings.id','courier_requests.pricing_id')->where('courier_requests.status_id','18')->where('delivery_hub','0')->where('branch_id',$branch->id)->where('hub_payment','yes')->where('paid_by','!=','sender')->sum('pricings.price');

                                    $pickup_hub_delivery_charge = \App\Models\CourierRequest::join('pricings','pricings.id','courier_requests.pricing_id')->where('courier_requests.status_id','18')->where('branch_id',$branch->id)->where('branch_id',$branch->id)->where('hub_payment','yes')->where('paid_by','sender')->sum('pricings.price');
                                ...
                                <td>
                                    <a href="{{ route('dashboard.account.paid.hub.lists.show',$branch->id) }}">
